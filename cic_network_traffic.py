...
data = pd.read_csv("data/cic_ids_2018.csv",index_col=0,header=0)
import os
...
freq_protocol = data['Label'].value_counts()
...
conversion_dict = {"Benign":0,  "DoS attacks-Hulk":1, "DoS attacks-GoldenEye":2,
                   'DDOS attack-HOIC':3}

data['Label'] = data['Label'].replace(conversion_dict)

values_to_delete = ['FTP-BruteForce',"SSH-Bruteforce","Infilteration","Bot","DoS attacks-SlowHTTPTest","DDOS attack-LOIC-UDP","DoS attacks-Slowloris"]
data = data[~data['Label'].isin(values_to_delete)]
print(data['Label'].value_counts())
...
#model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy',f1_metric])
